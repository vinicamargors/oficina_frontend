...
const selectClass =
  'w-full rounded-xl border border-zinc-800 bg-zinc-950 p-3.5 text-sm text-white outline-none focus:border-emerald-500/50 focus:ring-2 focus:ring-emerald-500/10 transition-colors appearance-none cursor-pointer';
...
/* ─── Color names for filter dots ─── */
const colorNames = ['branco', 'preto', 'prata', 'cinza', 'vermelho', 'azul', 'verde', 'amarelo', 'laranja', 'marrom', 'bege', 'dourado', 'roxo', 'rosa'];
      ...
      <div className="grid grid-cols-2 gap-2.5 mt-3">
        <div className="flex items-center gap-2">
          ...
          <div className="space-y-1">
            ...
            <div className="w-14 h-3 rounded bg-zinc-800/60" />
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-zinc-800/80" />
          <div className="space-y-1">
            <div className="w-6 h-2 rounded bg-zinc-800/40" />
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="space-y-1">
            <div className="w-6 h-2 rounded bg-zinc-800/40" />
            <div className="w-12 h-3 rounded bg-zinc-800/60" />
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-zinc-800/80" />
          <div className="space-y-1">
            <div className="w-10 h-2 rounded bg-zinc-800/40" />
            <div className="w-18 h-3 rounded bg-zinc-800/60" />
          </div>
        </div>
      </div>
      {/* Client row skeleton */}
...
export default function Veiculos() {
  ...
  /* ─── Available color filter dots (colors present in data) ─── */
  ...
  /* ─── Modal open/close ─── */
  const openNewModal = (prefilledClienteId?: string) => {
    ...
    setFormKm('');
    setFormChassi('');
    setFormClienteId(prefilledClienteId || '');
    setFormError('');
    setDuplicateWarning('');
    setModalOpen(true);
  };
  ...
  const openEditModal = (veiculo: Veiculo) => {
    ...
    setFormMarca(veiculo.marca);
    setFormCor(veiculo.cor);
    setFormAno(String(veiculo.ano));
    setFormKm(String(veiculo.km_atual));
    setFormChassi(veiculo.chassi);
    setFormClienteId(veiculo.cliente_id);
    setFormError('');
    setDuplicateWarning('');
    setModalOpen(true);
  };
  ...
  const checkDuplicatePlaca = (placa: string, currentId?: string): string => {
    ...
    return dup
      ? `Já existe um veículo com esta placa: ${dup.modelo} (${dup.placa})`
      : '';
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast.error('Erro ao salvar veículo.');
      setFormError(err instanceof Error ? err.message : 'Erro ao excluir veículo.');
    } finally {
      setDeleting(false);
    }
  };

  /* ─── Handle screen params for quick add ─── */
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-gradient-to-br from-zinc-900/80 to-zinc-900/40 border border-zinc-800/60 rounded-2xl p-1.5">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="relative">
            ...
            <select
              ...
              className={`${selectClass} pl-11 pr-10`}
            >
        ...
        {/* Quick filter pills: Color dots + Revisão Pendente */}
        <div className="flex items-center gap-3 mt-3 overflow-x-auto pb-1 custom-scrollbar">
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              ...
              {filterColor && (
                <button
                  ...
                  className="text-[10px] text-zinc-500 hover:text-white font-bold ml-1 flex-shrink-0"
                >
                  <X className="w-3 h-3" />
                </button>
              )}
          ...
          <button
            ...
            className={`flex-shrink-0 px-3 py-1.5 rounded-xl text-xs font-bold border transition-all duration-200 whitespace-nowrap ${
              filterReviewPending
      ...
      {filteredVeiculos.length === 0 ? (
        <div className="text-center py-20 relative">
          ...
          <div className="relative z-10">
            ...
            <p className="text-zinc-500 text-sm max-w-sm mx-auto mb-8">
              {veiculos.length === 0
                ? 'Adicione seu primeiro veículo para começar a gerenciar a garagem'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredVeiculos.map((veiculo) => {
            ...
            const proximaRevisao = veiculo.km_atual > 0
              ? (veiculo.km_atual + 5000).toLocaleString('pt-BR')
              : 'A verificar';
            ...
            return (
              <div
                key={veiculo.id}
                className={`group bg-zinc-900/50 border border-zinc-800 rounded-xl transition-all duration-300 ease-out hover:border-zinc-700 card-hover overflow-hidden relative ${
                ...
                <div className="p-4 md:p-6 cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : veiculo.id)}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      {/* Placa Badge + Revision indicator */}
                      ...
                      {/* Info grid: marca/cor, ano/km */}
                      <div className="grid grid-cols-2 gap-2.5 mt-3">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-lg bg-zinc-800/80 flex items-center justify-center flex-shrink-0">
                            <Tag className="w-3.5 h-3.5 text-zinc-400" />
                          </div>
                          <div>
                            <span className="uppercase tracking-wider text-[10px] text-zinc-500 font-bold block leading-none">Marca</span>
                            <span className="text-xs text-zinc-300 mt-0.5 block truncate">{veiculo.marca || '—'}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-lg bg-zinc-800/80 flex items-center justify-center flex-shrink-0">
                          ...
                          <div>
                            <span className="uppercase tracking-wider text-[10px] text-zinc-500 font-bold block leading-none">Cor</span>
                            <div className="flex items-center gap-1.5 mt-0.5">
                              <span className="w-4 h-4 rounded-full flex-shrink-0 border border-white/20 shadow-sm" style={{ backgroundColor: dotColor }} />
                              <span className="text-xs text-zinc-300 truncate">{veiculo.cor || '—'}</span>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <span className="uppercase tracking-wider text-[10px] text-zinc-500 font-bold block leading-none">Ano</span>
                            <span className="text-xs text-zinc-300 mt-0.5 block tabular-nums">{veiculo.ano}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-lg bg-zinc-800/80 flex items-center justify-center flex-shrink-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <span className="uppercase tracking-wider text-[10px] text-zinc-500 font-bold block leading-none">KM</span>
                            <span className="text-xs text-zinc-300 mt-0.5 block tabular-nums">{veiculo.km_atual?.toLocaleString('pt-BR') || '0'}</span>
                            {/* KM Progress Bar */}
                            {veiculo.km_atual > 0 && (
                              <div className="mt-1 h-1 w-full rounded-full bg-zinc-800 overflow-hidden">
                                <div
                                  className={`h-full rounded-full animate-stock-fill ${kmBarColor}`}
                      ...
                      {/* Próxima revisão with Calendar icon */}
                      ...
                      {/* Owner with User icon prefix */}
                      <div className="flex items-center gap-2.5 mt-3">
                        <div className="w-7 h-7 rounded-full bg-gradient-to-br from-emerald-500/20 to-emerald-600/5 border border-emerald-500/20 flex items-center justify-center flex-shrink-0">
                          <User className="w-3 h-3 text-emerald-400" />
                        </div>
                        <span className="text-zinc-300 text-xs font-medium truncate hover:text-emerald-400 transition-colors">
                          {veiculo.clientes?.nome || getClienteNome(veiculo.cliente_id)}
                        </span>
                      </div>
                    </div>

                    {/* Right: chevron + hover quick actions */}
                  ...
                  {/* Hover quick action buttons (non-expanded) */}
                  {!isExpanded && (
                    <div className="quick-actions-reveal absolute bottom-3 right-3 flex items-center gap-1.5">
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-zinc-800/90 border border-zinc-700/50 text-zinc-400 hover:text-white hover:border-zinc-600 transition-all"
                        title="Editar"
                      >
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          setExpandedId(veiculo.id);
                        }}
                        className="p-1.5 rounded-lg bg-zinc-800/90 border border-zinc-700/50 text-zinc-400 hover:text-emerald-400 hover:border-emerald-500/30 transition-all"
                        title="Detalhes"
                      >
                  ...
                  <div className="border-t border-zinc-800/50 px-4 md:px-6 py-4 space-y-4 animate-in fade-in slide-in-from-top-2 transition-all duration-300 ease-out">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-white text-sm mt-0.5">{veiculo.modelo}</p>
                      </div>
                      <div>
                        <span className="uppercase tracking-wider text-[10px] text-zinc-500 font-bold block mb-1">
                          Marca
                        </span>
                        <p className="text-white text-sm mt-0.5">{veiculo.marca}</p>
                      </div>
                      <div>
                        <span className="uppercase tracking-wider text-[10px] text-zinc-500 font-bold block mb-1">
                          Ano
                        </span>
                        <p className="text-white text-sm mt-0.5 tabular-nums">{veiculo.ano}</p>
                      </div>
                      <div>
                        <span className="uppercase tracking-wider text-[10px] text-zinc-500 font-bold block mb-1">
                          Cor
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <span className="w-4 h-4 rounded-full border border-white/20 shadow-sm" style={{ backgroundColor: dotColor }} />
                          <p className="text-white text-sm">{veiculo.cor}</p>
                        </div>
                      </div>
                      <div>
                        <span className="uppercase tracking-wider text-[10px] text-zinc-500 font-bold block mb-1">
                        ...
                        <p className="text-white text-sm mt-0.5 tabular-nums">{veiculo.km_atual?.toLocaleString('pt-BR') || '0'} km</p>
                      </div>
                      {veiculo.chassi && (
                        <div className="col-span-2">
                          ...
                          <p className="text-white text-sm mt-0.5 font-mono">{veiculo.chassi}</p>
                        </div>
                      )}
                    </div>
                    ...
                    {cliente && (
                      <div className="bg-zinc-800/30 border border-zinc-700/30 rounded-xl p-4">
                        <span className="uppercase tracking-wider text-[10px] text-zinc-500 font-bold block mb-2.5">
                          Proprietário
                        </span>
                        <div className="flex items-center justify-between gap-3">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-emerald-500/20 to-emerald-600/5 border border-emerald-500/20 flex items-center justify-center flex-shrink-0">
                          ...
                          {cliente.telefone && (
                            <a
                              href={`https://wa.me/55${cleanPhone(cliente.telefone)}`}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="inline-flex items-center gap-1.5 bg-emerald-600/15 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-600/25 font-bold rounded-xl px-3 py-2 text-xs transition-colors"
                            >
                    ...
                    <div className="flex items-center gap-2 text-zinc-500 text-xs">
                      ...
                      <span>
                        Veja o histórico completo na tela de{' '}
                        <button
                    ...
                    {deleteConfirm === veiculo.id ? (
                      <div className="bg-red-500/5 border border-red-500/20 rounded-xl p-4">
                        <p className="text-red-300 text-sm font-medium mb-3">
                          Excluir veículo <strong className="font-mono">{veiculo.placa.toUpperCase()}</strong> ({veiculo.modelo})?
                        </p>
                        <div className="flex items-center gap-2">
                          <button
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => setDeleteConfirm(veiculo.id)}
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          value={formPlaca}
                          onChange={(e) => {
                            const val = maskPlaca(e.target.value);
                            setFormPlaca(val);
                            setDuplicateWarning('');
                          }}
                          onBlur={handlePlacaBlur}
                          className={`${inputClass} font-mono uppercase tracking-wider ${placaLookupLoading ? 'pr-10' : ''}`}
                          maxLength={8}
                          autoFocus
                        />
                        {placaLookupLoading && (
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={formChassi}
                      onChange={(e) => setFormChassi(e.target.value.toUpperCase())}
